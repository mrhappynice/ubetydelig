...
def process_command(command):
    ...
    Process a command.
    Supported formats:
      - "move to b3" or "move to c16" (letter followed by 1-2 digit number)
    ...
    if command.startswith("move to"):
        ...
        if match:
            ...
            row_index = number - 1
            # Compute center of the selected cell
...
class GridWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        pen = QPen(QColor(0, 0, 0, 150))  # Black with some transparency
        pen.setWidth(1)
        painter.setPen(pen)
